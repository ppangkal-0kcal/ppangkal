...
## 앱 접근권한 사전 고지 (2026-09-16, 스토어 피드백 대응)

- 정보통신망법 제22조의2 / 방송미디어통신위원회 「앱 접근권한 동의 가이드라인」. 원스토어에서
  "접근권한을 요구하나 사전 고지·동의 절차가 없다"는 지적을 받아 추가했다.
- **최초 실행 시 로그인보다 먼저** `/permissions`(`screens/permission_notice_screen.dart`)를
  한 번 띄운다. 회원가입 화면이 아니라 로그인 앞에 둔 이유: 기존 계정으로 로그인만 하는
  사용자와 심사자가 회원가입 화면을 거치지 않기 때문이다.
- 고지 내용은 `models/app_permission.dart`의 `appPermissions` 한 곳에서 온다.
  **AndroidManifest의 런타임 권한과 1:1로 맞춰야 한다** — 권한을 추가·삭제하면 이 목록도
  같이 고쳐야 고지가 실제 동작과 어긋나지 않는다.
- 지금은 세 권한(위치·신체 활동·알림) 모두 **선택적**이다. 거부해도 앱이 동작하므로
  "필수 권한 없음"으로 고지한다. 필수 권한이 생기면 화면이 "필수" 묶음을 자동으로 그린다.
- 본 여부는 `PermissionNoticeProvider`가 `flutter_secure_storage`에 기기 로컬로만 기록한다.
- 가이드라인이 언제든 다시 볼 수 있을 것을 요구해서 마이페이지에도 진입점을 뒀다.
